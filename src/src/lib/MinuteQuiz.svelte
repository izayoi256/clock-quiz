...
const nextLevel = () => {
  ...
  while (true) {
    const minute = correctMinute + Math.floor(Math.random() * 31) - 15;
    if (minute === correctMinute || minute < 0 || 59 < minute ) {
      continue;
    }
    newAnswersMap.set(minute.toString(), [minute, false]);
    if (newAnswersMap.size > 3) {
      break;
    }
  }
  const newChoices = Array.from(newAnswersMap.values());
  newChoices[Math.floor(Math.random() * newAnswersMap.size)] = [correctMinute, true];
  choices = newChoices;
  chose = false;
}
